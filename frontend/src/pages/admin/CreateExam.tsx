...
const CreateExam: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      present({ message: 'Exam created successfully!', duration: 2000, position: 'bottom', color: 'success' });
      setFormData({
        title: '',
        description: '',
        duration: '',
      ...
      <IonContent fullscreen className="ion-padding">
        <form className="space-y-4 max-w-2xl mx-auto">
          ...
          <div className="flex gap-2 pt-4">
            <IonButton expand="block" fill="clear" color="medium" onClick={() => history.push('/exams')} disabled={loading}>Cancel</IonButton>
            <IonButton expand="block" fill="solid" color="primary" onClick={handleSubmit} disabled={loading}>{loading ? <IonSpinner name="crescent" /> : 'Create Exam'}</IonButton>
          </div>
        </form>
      </IonContent>
    </IonPage>

...
const SuperAdminUsers: React.FC = () => {
  ...
  const handleRoleFilterChange = (value: string) => {
    ...
    setLoading(true);
    userApi.getAllUsers({ page: 1, limit: 10, search: searchTerm || undefined, role: value || undefined })
      .then((res) => {
        setUsers(res.users ?? []);
        setPagination(res.pagination ?? { page: 1, limit: 10, totalUsers: 0, totalPages: 0 });
      })
    ...
    presentAlert({
      ...
      buttons: [
        { text: 'Cancel', role: 'cancel' },
        {
          ...
          handler: async () => {
            try {
              await userApi.deleteSuperUserById(user._id);
  ...
  const handleSaveEdit = async () => {
    ...
    setEditLoading(true);
    try {
      await userApi.updateSuperUserById(editingUser._id, { ...editForm });
  ...
  // ── Create user ──────────────────────────────────────────────────────────
  const resetForm = () =>
    setForm({ name: '', email: '', password: '', ranNo: '', role: '', phone: '', association: '', conference: '' });
  ...
  const handleCreate = async () => {
    ...
    try {
      await userApi.createSuperUser({ ...form });
          ...
          <div className="mx-auto max-w-5xl space-y-5">
            ...
            <div className="rounded-2xl bg-white p-5 shadow-sm">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                >
                  <IonIcon icon={personAdd} slot="start" className='mr-2' />
                   Add User
                </IonButton>
              </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <div className="rounded-xl bg-sky-50 px-4 py-3">
                  <p className="text-xs text-sky-400">Examiners</p>
                  <p className="mt-0.5 text-xl font-bold text-sky-700">
                    {users.filter((u) => u.role === 'examiner').length}
                  </p>
                </div>
                <div className="rounded-xl bg-purple-50 px-4 py-3">
                  <p className="text-xs text-purple-400">Candidates</p>
                  <p className="mt-0.5 text-xl font-bold text-purple-700">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-slate-400 truncate">{`${user?.organizationId?.name || 'Super Admin'} (${user?.organizationId?.organizationCode || '0000'})`}</p>
                          <p className="text-xs text-slate-400 font-mono">RAN No.: {user.ranNo}</p>
                        </div>
                      </div>
